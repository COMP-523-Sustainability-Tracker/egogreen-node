...
export class Categorizer {
  constructor(data) {
    // SQLite only for United Nations Standard Products and Services Code
    this.db = new sqlite3.Database(process.env.DBFILE, (err) => {
        if (err) {console.error(err.message)}
    })

    this.receiptData = {}
    this.receiptData.processedTime = new Date().toISOString()
    ...
    const paymentType = (!data.hasOwnProperty("paymentType") ? data.paymentType.data : " " )
    this.receiptData.paymentType = paymentType
    ...
    data.entities.productLineItems.forEach(element => {
      if(element.data.hasOwnProperty("unitPrice")) {
        // If we don't have a unit price we can not proceed with this line item
...
// OpenAI Code
  buildOpenAIQuery(item, categories){
    ...
    item +
    " purchased from " + this.receiptData.merchantName +  // Was confusing openAI
    " from the following options: " + 
    categories +
    " answer in JSON with the fields item and category. Respond with none in the category field if unable to classify."
    return openAIQuery
  }
