...
export class Categorizer {
  constructor(data) {
    ...
    data.entities.productLineItems.forEach(element => {
      const qty = (!element.hasOwnProperty("data.quantity.data"))  ? 1 : element.data.quantity.data
      const totalAmount = (!element.hasOwnProperty("data.totalPrice.data")) ? parseFloat(element.data.unitPrice.data)*qty : element.data.totalPrice.data
...
async getCode(sql, params) {
  try {
    const row = await this.dbGetPromise(sql, params)
    if (!row) {
      console.log(params)
      return '50000000'  //Hack for when there are no rows
    }      
    return row.Code
  } catch (error) {
    console.log(error)
...
// OpenAI Code
  ...
  async setTotal() {
    ...
    this.receiptData.calculatedTotal = parseFloat(this.receiptData.calculatedSubTotal) + parseFloat(this.receiptData.taxAmount)
    this.receiptData.totalCO2e = totalCO2e
    return true
  }
}
